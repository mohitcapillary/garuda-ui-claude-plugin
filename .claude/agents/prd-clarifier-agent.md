...
- **All Figma URLs must appear in output.** Extract every Figma URL from the PRD with its screen label (text preceding the URL, e.g., "Listing", "Create"). List all of them in the header and appendix with full URLs — never collapse multiple links into one.
...
- **ID-to-name resolution check.** For every ID field in a BE response (e.g., `updatedBy`, `createdBy`, `programId`), check whether the corresponding Figma column renders a human-readable name instead of the raw ID. If so, the PRD must specify which API resolves that ID to a display name — or BE must add a resolved name field. Raise one `Backend` question per unresolved ID-to-name mapping. Do not stop after finding one — check **every** ID field systematically.
- **`If unanswered` defaults must prefer existing APIs.** Before assuming BE must add a new field or endpoint, check `app/services/api.js` for existing service functions that already solve the problem (e.g., `getUsersByIds` for user ID → name, `getPrograms` for program ID → name). Reference the real function name and endpoint in the default.
