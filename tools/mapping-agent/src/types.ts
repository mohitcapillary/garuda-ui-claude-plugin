...
export interface CompositeChild {
  figmaLayerName: string; // exact name or glob pattern
  targetSlot?: string; // which slot of parent this child fills
}

export interface ComponentMapping {
  figmaIdentifier: string;
  nodeTypes: FigmaNodeType[];
  componentNames: string[]; // glob patterns, case-insensitive
  variantPatterns?: Record<string, string>;
  targetComponent: string;
  importPath: string;
  defaultProps?: Record<string, unknown>; // always applied when this entry matches
...
export interface TokenMapping {
  ...
  figmaValuePattern?: string; // hex or numeric value for value-based fallback lookup
  blazeCSSVar: string;
  blazeTSConst?: string;
  tokenType: TokenType;
}
...
export type ResolutionSource =
  ...
  | 'typography'      // TEXT node → CapLabel
  | 'vision-inferred' // resolved by Claude vision API from screenshot crop
  | 'unresolved';     // UNMAPPED with no resolution
...
// ─── Chunked Fetch / Screen Manifest Types ───────────────────────────────────
