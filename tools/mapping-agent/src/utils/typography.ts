...
const LABEL_MATRIX: Array<[number, number, string]> = [
 ...
 */
export function findLabelType(fontSize: number, fontWeight: number): string {
  // Find the first entry that matches both size and weight
  const match = LABEL_MATRIX.find(
    ([size, weight]) => size === fontSize && weight === fontWeight
  );
  if (match) return match[2];

  // Fallback: match size only, prefer medium weight for 500
  const sizeMatch = LABEL_MATRIX.find(([size]) => size === fontSize);
  if (sizeMatch) return sizeMatch[2];
 ...
 */
export function findFontSizeToken(px: number): string {
 ...
 */
export function mapFontToNonRoboto(fontFamily: string): string | null {
